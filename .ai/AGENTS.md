...
Proto definitions are split across two files in the same `silo.v1` package:

- `proto/silo.proto` - Core worker-facing service (`Silo`): enqueue, lease, heartbeat, query, etc.
- `proto/silo_admin.proto` - Admin/operator service (`SiloAdmin`): splits, compaction, node info, profiling, imports, etc. Used by the webui and siloctl.
- `proto/gubernator.proto` - Rate limiting (Gubernator client).

Rust code is generated by `build.rs` during cargo build. If you modify proto files, just run `cargo build` - no separate protoc step needed. When adding a new RPC, put it in the correct service based on whether it's worker-facing or operator-facing.
...
The webui is served by any node but must work correctly for **all** shards, not just locally-owned ones. Never access `ShardFactory` directly from a webui handler to read shard data — always go through `ClusterClient`, which routes to the owning node via gRPC when the shard is remote. If a new feature needs data from a `JobStoreShard`, add or extend a gRPC RPC so the cluster client can fetch it from any node, then use that cluster client method in the webui handler.
