 ...
 */
function isLeaseGoneError(error) {
    if (error instanceof client_1.TaskNotFoundError) {
        return error.grpcMessage?.includes("lease not found") ?? false;
    }
    if (error instanceof client_1.SiloFailedPreconditionError) {
...
class SiloWorker {
    ...
    _enqueueTask(protoTask) {
        ...
        if (this._activeExecutions.has(task.id)) {
            ...
            this._onError(new client_1.DuplicateTaskDeliveryError(task.id, task.jobId), { taskId: task.id });
            return;
        }
    ...
    _enqueueRefreshTask(task) {
        ...
        if (this._heartbeatIntervals.has(task.id)) {
            this._metrics.duplicateTaskDeliveriesCounter.add(1, this._metrics.defaultAttributes);
            this._onError(new client_1.DuplicateTaskDeliveryError(task.id), { taskId: task.id });
            return;
        }
     ...
     */
    async _executeTaskWithExecution(execution, stopHeartbeat) {
     ...
     */
    async _executeRefreshTask(task, shard, stopHeartbeat) {
        const context = {
            task,
            shard,
            workerId: this._workerId,
     ...
     */
    async _sendHeartbeatForTask(execution, stopHeartbeat) {
        ...
        let result;
        try {
            result = await this._client.heartbeat(this._workerId, execution.task.id, execution.task.shard, execution.task.tenantId);
        }
        catch (error) {
            ...
            if (isLeaseGoneError(error)) {
                this._handleLeaseLost(execution, error, stopHeartbeat);
                return;
            }
     ...
     * the event once.
     */
    _handleLeaseLost(execution, cause, stopHeartbeat) {
        stopHeartbeat();
        ...
        this._onError(new client_1.TaskLeaseLostError(execution.task.id, execution.task.jobId, cause), {
            taskId: execution.task.id,
        });
    }
    /**
