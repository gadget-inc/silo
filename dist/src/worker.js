...
class TaskExecution {
    ...
    _client;
    constructor(task, shard, workerId, tenant, workerAbortSignal, client) {
        this.task = task;
        this.shard = shard;
        ...
        this._combinedSignal = combineAbortSignals(workerAbortSignal, this._taskAbortController.signal);
...
class SiloWorker {
    ...
    _enqueueTask(task) {
        ...
        const execution = new TaskExecution(task, shard, this._workerId, this._tenant, this._abortController?.signal ?? new AbortController().signal, this._client);
    ...
    async _sendHeartbeatForTask(execution) {
        ...
        const result = await this._client.heartbeat(this._workerId, execution.task.id, execution.shard, this._tenant);
