...
class SiloWorker {
    ...
    _enqueueTask(task) {
        ...
        const execution = new TaskExecution_1.TaskExecution(task, shard, this._workerId, this._abortController?.signal ?? new AbortController().signal, this._client);
    ...
    async _sendHeartbeatForTask(execution) {
        ...
        const result = await this._client.heartbeat(this._workerId, execution.task.id, execution.shard);
