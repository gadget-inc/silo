...
class TaskNotFoundError extends Error {
     ...
     */
    grpcMessage;
    constructor(taskId, grpcMessage) {
...
exports.TaskLeaseLostError = TaskLeaseLostError;
/**
...
exports.DuplicateTaskDeliveryError = DuplicateTaskDeliveryError;
/**
...
function initHasher() {
    if (!_initPromise) {
        _initPromise = (0, xxhash_wasm_1.default)().then((api) => {
            _h64 = api.h64;
        });
    }
    return _initPromise;
 ...
 */
function shardForTenant(key, shards) {
    if (shards.length === 0) {
        return undefined;
    }
...
function toProtoQueryParameter(value) {
    if (value === null) {
    ...
    if (typeof value === "number") {
        ...
        if (Number.isInteger(value)) {
            if (!Number.isSafeInteger(value)) {
                throw new Error("query bind integer parameters must be safe integers; use bigint for larger values");
            }
...
function toProtoQueryParameters(parameters) {
 ...
 * @internal
 */
function publicAttemptStatusToProto(status) {
    switch (status) {
        case AttemptStatus.Running:
            return silo_1.AttemptStatus.RUNNING;
        case AttemptStatus.Succeeded:
            return silo_1.AttemptStatus.SUCCEEDED;
        case AttemptStatus.Failed:
            return silo_1.AttemptStatus.FAILED;
        case AttemptStatus.Cancelled:
            return silo_1.AttemptStatus.CANCELLED;
        default: {
            const _exhaustive = status;
            throw new Error(`Unknown attempt status: ${String(_exhaustive)}`);
        }
 ...
 */
const tracePropagationInterceptor = {
...
function injectTraceContext(rpcOptions) {
    if (!rpcOptions.meta) {
        rpcOptions.meta = {};
    }
    ...
    api_1.propagation.inject(api_1.context.active(), rpcOptions.meta, {
...
class SiloGRPCClient {
    ...
    constructor(options) {
        ...
        if (useTls && tokenFn) {
            this._channelCredentials = grpc_js_1.credentials.combineChannelCredentials(grpc_js_1.ChannelCredentials.createSsl(), grpc_js_1.credentials.createFromMetadataGenerator((_, callback) => {
                tokenFn()
                    .then((token) => {
            ...
            this._channelCredentials = grpc_js_1.ChannelCredentials.createInsecure();
        ...
        // Metrics
        const meter = options.meter ?? api_1.metrics.getMeter("silo-client");
    ...
    _getOrCreateConnection(address) {
        ...
        if (!conn) {
            const interceptors = [tracePropagationInterceptor];
     ...
     * @internal
     */
    async _retryLoop(resolveClient, operation) {
        let lastError;
        ...
        for (let attempt = 0; attempt <= this._maxRetries; attempt++) {
            try {
                ...
                if ((isWrongShardError(error) || isConnectivityError(error)) &&
     ...
     * @internal
     */
    async _withShardRetry(tenant, operation) {
     ...
     * @internal
     */
    async _withShardRetryForShard(shardId, operation) {
        return this._retryLoop(() => ({ client: this._getClientForShard(shardId), shard: shardId }), (client) => operation(client));
    }
    /**
            ...
            return await this._withShardRetry(tenant, async (client, shard) => {
                ...
                return {
                    ...
                    result: response.result
                        ? tolerantDecodeAttemptBytes(response.result.encoding.oneofKind === "msgpack"
            ...
            return await this._withShardRetry(tenant, async (client, shard) => {
                ...
                if (status === JobStatus.Succeeded) {
                    ...
                    if (response.result.oneofKind === "successData" &&
                        ...
                        result = tolerantDecodeAttemptBytes(response.result.successData.encoding.msgpack, "successData");
                    }
                ...
                if (status === JobStatus.Failed) {
                    ...
                    if (response.result.oneofKind === "failure" &&
                        ...
                        errorData = tolerantDecodeAttemptBytes(response.result.failure.errorData.encoding.msgpack, "failure");
                    }
                    return {
    ...
    async leaseTasks(options, serverIndex) {
        const executeLeaseRpc = async (client) => {
        ...
        try {
            ...
            return await this._retryLoop(() => {
                const client = serverIndex !== undefined ? this._getClientAtIndex(serverIndex) : this._getAnyClient();
                return { client, shard: "" };
     ...
     */
    async query(sql, tenant, parameters) {
        try {
            return await this._withShardRetry(tenant, async (client, shard) => {
 ...
 */
function tolerantDecodeAttemptBytes(bytes, _field) {
    if (!bytes || bytes.length === 0) {
        return undefined;
    }
    try {
