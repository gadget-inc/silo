...
const silo_1 = require("../src/pb/silo");
...
(0, vitest_1.describe)("tolerantDecodeAttemptBytes", () => {
    (0, vitest_1.it)("decodes valid msgpack bytes normally", () => {
        const bytes = (0, client_1.encodeBytes)({ foo: "bar", count: 42 });
        (0, vitest_1.expect)((0, client_1.tolerantDecodeAttemptBytes)(bytes, "errorData")).toEqual({ foo: "bar", count: 42 });
    });
    (0, vitest_1.it)("falls back to the UTF-8 string for undecodable bytes", () => {
        const raw = new TextEncoder().encode("lease expired at 123 (now 456), worker=w1");
        (0, vitest_1.expect)((0, client_1.tolerantDecodeAttemptBytes)(raw, "errorData")).toBe("lease expired at 123 (now 456), worker=w1");
    });
    (0, vitest_1.it)("returns undefined for empty bytes", () => {
        (0, vitest_1.expect)((0, client_1.tolerantDecodeAttemptBytes)(new Uint8Array(0), "errorData")).toBeUndefined();
    });
    (0, vitest_1.it)("returns undefined for undefined bytes", () => {
        (0, vitest_1.expect)((0, client_1.tolerantDecodeAttemptBytes)(undefined, "errorData")).toBeUndefined();
    });
});
(0, vitest_1.describe)("SiloGRPCClient", () => {
    ...
    (0, vitest_1.describe)("error mapping", () => {
        const mockWithShardRetryError = (client, code, message) => {
            client._withShardRetry = vitest_1.vi.fn().mockRejectedValue(new runtime_rpc_1.RpcError(message, code, {}));
        };
    ...
    (0, vitest_1.describe)("attempt blob decode tolerance", () => {
        const rawErrorText = "lease expired at 123 (now 456), worker=w1";
        const rawErrorBytes = new TextEncoder().encode(rawErrorText);
        const msgpackBytes = (value) => ({
            encoding: { oneofKind: "msgpack", msgpack: (0, client_1.encodeBytes)(value) },
        });
        const protoAttempt = (attemptNumber, overrides) => ({
            jobId: "job-1",
            attemptNumber,
            taskId: `task-${attemptNumber}`,
            status: silo_1.AttemptStatus.FAILED,
            startedAtMs: 1n,
            finishedAtMs: 2n,
        ...
        const jobResponse = (overrides) => ({
            ...
            limits: [],
            metadata: {},
            status: silo_1.JobStatus.FAILED,
        ...
        const stubTransport = (client, methods) => {
            client._ensureTopology = vitest_1.vi.fn().mockResolvedValue(undefined);
            client._getClientForTenant = vitest_1.vi
                .fn()
                .mockReturnValue({ shard: "shard-1", client: methods });
        };
        (0, vitest_1.it)("returns all attempts when one attempt's errorData is not msgpack", async () => {
            ...
            stubTransport(client, {
                getJob: vitest_1.vi.fn().mockReturnValue({ response: Promise.resolve(response) }),
            });
            const job = await client.getJob("job-1", "tenant-a", { includeAttempts: true });
            (0, vitest_1.expect)(job.attempts).toHaveLength(2);
            (0, vitest_1.expect)(job.attempts?.[0].errorData).toBe(rawErrorText);
            (0, vitest_1.expect)(job.attempts?.[1].errorData).toEqual({ code: "E", message: "boom" });
        });
        (0, vitest_1.it)("returns result undefined for a Succeeded job with zero-length result bytes", async () => {
            const client = createClient({ servers: "localhost:7450", ...defaultOptions });
            const response = jobResponse({
                status: silo_1.JobStatus.SUCCEEDED,
                result: { encoding: { oneofKind: "msgpack", msgpack: new Uint8Array(0) } },
                attempts: [
                    protoAttempt(1, {
                        status: silo_1.AttemptStatus.SUCCEEDED,
                        errorCode: undefined,
                        result: { encoding: { oneofKind: "msgpack", msgpack: new Uint8Array(0) } },
                    }),
                ],
            ...
            const job = await client.getJob("job-1", "tenant-a", { includeAttempts: true });
            (0, vitest_1.expect)(job.result).toBeUndefined();
            (0, vitest_1.expect)(job.attempts?.[0].result).toBeUndefined();
        });
        (0, vitest_1.it)("returns the UTF-8 string when getJobResult failure data is not msgpack", async () => {
            const client = createClient({ servers: "localhost:7450", ...defaultOptions });
            stubTransport(client, {
                getJobResult: vitest_1.vi.fn().mockReturnValue({
            ...
            (0, vitest_1.expect)(result.errorCode).toBe("WORKER_CRASHED");
            (0, vitest_1.expect)(result.errorData).toBe(rawErrorText);
        });
    });
});
...
(0, vitest_1.describe)("SiloGRPCClient.query deserialization", () => {
    ...
    (0, vitest_1.it)("deserializes msgpack rows in query response", async () => {
        ...
        client._withShardRetry = vitest_1.vi
            .fn()
    ...
    (0, vitest_1.it)("returns typed rows with generic parameter", async () => {
        ...
        client._withShardRetry = vitest_1.vi.fn().mockImplementation(async () => {
            return {
...
(0, vitest_1.describe)("isConnectivityError", () => {
    (0, vitest_1.it)("returns true for UNAVAILABLE", () => {
        const error = new runtime_rpc_1.RpcError("server unavailable", "UNAVAILABLE", {});
        (0, vitest_1.expect)((0, client_1.isConnectivityError)(error)).toBe(true);
    });
    (0, vitest_1.it)("returns true for DEADLINE_EXCEEDED", () => {
        const error = new runtime_rpc_1.RpcError("deadline exceeded", "DEADLINE_EXCEEDED", {});
        (0, vitest_1.expect)((0, client_1.isConnectivityError)(error)).toBe(true);
    });
    (0, vitest_1.it)("returns true for INTERNAL with RST_STREAM message", () => {
        const error = new runtime_rpc_1.RpcError("Received RST_STREAM with code 5", "INTERNAL", {});
        (0, vitest_1.expect)((0, client_1.isConnectivityError)(error)).toBe(true);
    });
    (0, vitest_1.it)("returns true for INTERNAL with RST_STREAM code 2", () => {
        const error = new runtime_rpc_1.RpcError("Received RST_STREAM with code 2", "INTERNAL", {});
        (0, vitest_1.expect)((0, client_1.isConnectivityError)(error)).toBe(true);
    });
    (0, vitest_1.it)("returns false for INTERNAL without RST_STREAM", () => {
        const error = new runtime_rpc_1.RpcError("internal server error", "INTERNAL", {});
        (0, vitest_1.expect)((0, client_1.isConnectivityError)(error)).toBe(false);
    });
    (0, vitest_1.it)("returns false for NOT_FOUND", () => {
        const error = new runtime_rpc_1.RpcError("not found", "NOT_FOUND", {});
        (0, vitest_1.expect)((0, client_1.isConnectivityError)(error)).toBe(false);
    });
    (0, vitest_1.it)("returns false for ALREADY_EXISTS", () => {
        const error = new runtime_rpc_1.RpcError("already exists", "ALREADY_EXISTS", {});
        (0, vitest_1.expect)((0, client_1.isConnectivityError)(error)).toBe(false);
    });
    (0, vitest_1.it)("returns false for RESOURCE_EXHAUSTED", () => {
        const error = new runtime_rpc_1.RpcError("exhausted", "RESOURCE_EXHAUSTED", {});
        (0, vitest_1.expect)((0, client_1.isConnectivityError)(error)).toBe(false);
    });
    (0, vitest_1.it)("returns false for non-RpcError", () => {
        (0, vitest_1.expect)((0, client_1.isConnectivityError)(new Error("generic"))).toBe(false);
        (0, vitest_1.expect)((0, client_1.isConnectivityError)("string error")).toBe(false);
        (0, vitest_1.expect)((0, client_1.isConnectivityError)(null)).toBe(false);
        (0, vitest_1.expect)((0, client_1.isConnectivityError)(undefined)).toBe(false);
    });
});
