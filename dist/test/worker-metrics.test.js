...
function getMetricAttributes(allMetrics, name) {
    const points = allMetrics[name];
    if (!points || points.length === 0)
        return {};
...
(0, vitest_1.describe)("SiloWorker metrics", () => {
    ...
    (0, vitest_1.it)("increments duplicate task deliveries counter when a running task is re-delivered", async () => {
        ...
        await worker.stop();
        const allMetrics = await collectMetrics(metricReader);
        (0, vitest_1.expect)(getMetricValue(allMetrics, "silo.worker.duplicate_task_deliveries")).toBe(1);
        (0, vitest_1.expect)(getMetricAttributes(allMetrics, "silo.worker.duplicate_task_deliveries")).toEqual({
            task_group: "default",
        });
    });
    (0, vitest_1.it)("includes task_group label on all metrics", async () => {
        let pollCount = 0;
        const leaseTasks = vitest_1.vi.fn().mockImplementation(async () => {
            pollCount++;
            return tasksResult([]);
        ...
        await worker.stop();
        const allMetrics = await collectMetrics(metricReader);
        (0, vitest_1.expect)(getMetricAttributes(allMetrics, "silo.worker.polls")).toEqual({
            task_group: "my-task-group",
        });
        (0, vitest_1.expect)(getMetricAttributes(allMetrics, "silo.worker.polls.empty")).toEqual({
            task_group: "my-task-group",
        });
        (0, vitest_1.expect)(getMetricAttributes(allMetrics, "silo.worker.available_task_slots")).toEqual({
            task_group: "my-task-group",
        });
    });
