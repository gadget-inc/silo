...
vitest_1.describe.skipIf(!RUN_INTEGRATION)("SiloWorker integration", () => {
    ...
    (0, vitest_1.describe)("abort signal", () => {
        (0, vitest_1.it)("provides abort signal that is only aborted on cancellation, not worker stop", async () => {
            ...
            (0, vitest_1.expect)(signalAborted).toBe(false);
        });
    });
    (0, vitest_1.describe)("shard context", () => {
        (0, vitest_1.it)("provides correct shard in task context", async () => {
            ...
            (0, vitest_1.expect)(typeof receivedShard).toBe("string");
            (0, vitest_1.expect)(receivedShard.length).toBeGreaterThan(0);
        });
    });
    (0, vitest_1.describe)("task group isolation", () => {
        (0, vitest_1.it)("worker polling default task group does not receive tasks from other task groups", async () => {
            const processedByDefaultWorker = [];
            ...
            (0, vitest_1.expect)(processedByDefaultWorker).not.toContain("task-for-other-group");
            (0, vitest_1.expect)(processedByDefaultWorker.length).toBe(0);
            // Verify the job is still scheduled (not picked up)
            const job = await client.getJob(handle.id, DEFAULT_TENANT);
            (0, vitest_1.expect)(job.status).toBe(client_1.JobStatus.Scheduled);
        });
        (0, vitest_1.it)("worker polling a specific task group receives tasks from that task group", async () => {
            const processedBySpecificWorker = [];
            const specificTaskGroup = `specific-group-${Date.now()}`;
            // Worker polling the specific task group
            const specificHandler = async (ctx) => {
            ...
            await waitFor(async () => {
                const job = await client.getJob(handle.id, DEFAULT_TENANT);
                return job.status === client_1.JobStatus.Succeeded;
            });
            // Verify the task was processed by our handler
            (0, vitest_1.expect)(processedBySpecificWorker).toContain("task-for-specific-group");
            // Verify the job completed successfully (already waited for this above)
            const job = await client.getJob(handle.id, DEFAULT_TENANT);
            (0, vitest_1.expect)(job.status).toBe(client_1.JobStatus.Succeeded);
        });
        (0, vitest_1.it)("multiple workers on different task groups only receive their own tasks", async () => {
            ...
            await waitFor(() => processedByWorkerA.includes("task-for-A") &&
                processedByWorkerB.includes("task-for-B"), { timeout: 10000 });
            // Each worker should only have processed its own task group's tasks
            (0, vitest_1.expect)(processedByWorkerA).toContain("task-for-A");
            (0, vitest_1.expect)(processedByWorkerA).not.toContain("task-for-B");
            (0, vitest_1.expect)(processedByWorkerB).toContain("task-for-B");
            (0, vitest_1.expect)(processedByWorkerB).not.toContain("task-for-A");
        }, 15000);
    });
    (0, vitest_1.describe)("floating concurrency limit refresh", () => {
        function createWorkerWithRefresh(handler, refreshHandler, options) {
            const worker = new worker_1.SiloWorker({
                client,
        ...
        (0, vitest_1.it)("processes refresh tasks with refreshHandler", async () => {
            const refreshedQueues = [];
            const processedJobs = [];
            ...
            (0, vitest_1.expect)(refreshedQueues).toContain(queueKey);
        });
        (0, vitest_1.it)("refresh handler receives correct context", async () => {
            ...
            (0, vitest_1.expect)(receivedContext).not.toBeNull();
            (0, vitest_1.expect)(receivedContext.queueKey).toBe(queueKey);
            (0, vitest_1.expect)(receivedContext.currentMax).toBe(10);
            (0, vitest_1.expect)(receivedContext.metadata).toEqual(testMetadata);
            (0, vitest_1.expect)(typeof receivedContext.shard).toBe("string");
            (0, vitest_1.expect)(receivedContext.shard.length).toBeGreaterThan(0);
            (0, vitest_1.expect)(receivedContext.workerId).toBeTruthy();
        });
        (0, vitest_1.it)("handles refresh handler errors gracefully", async () => {
        ...
        (0, vitest_1.it)("processes both job tasks and refresh tasks concurrently", async () => {
            const processedJobs = [];
            const refreshedQueues = [];
            ...
            (0, vitest_1.expect)(processedJobs.length).toBeGreaterThan(0);
            // Note: refresh task processing depends on timing, so we don't strictly require it
        });
        (0, vitest_1.it)("errors when receiving refresh tasks without a refreshHandler configured", async () => {
            const errors = [];
            ...
            (0, vitest_1.expect)(errors[0].message).toContain("refreshHandler");
            (0, vitest_1.expect)(errors[0].message).toContain("floating limit refresh task");
            await worker.stop();
        });
    });
    (0, vitest_1.describe)("task cancellation", () => {
        (0, vitest_1.it)("aborts signal when server cancels job during heartbeat", async () => {
            let taskStarted = false;
            let signalAborted = false;
            const handler = async (ctx) => {
                taskStarted = true;
            ...
            await waitFor(async () => {
                const job = await client.getJob(handle.id, DEFAULT_TENANT);
                return job.status === client_1.JobStatus.Cancelled;
            }, { timeout: 5000 });
            const finalJob = await client.getJob(handle.id, DEFAULT_TENANT);
            (0, vitest_1.expect)(finalJob.status).toBe(client_1.JobStatus.Cancelled);
        });
        (0, vitest_1.it)("reports Cancelled outcome when job is cancelled during execution", async () => {
            let taskStarted = false;
            let handlerReturnedValue = null;
            const handler = async (ctx) => {
                taskStarted = true;
        ...
        (0, vitest_1.it)("handler can cancel task via context.cancel()", async () => {
            let taskStarted = false;
            ...
            (0, vitest_1.expect)(taskStarted).toBe(true);
            (0, vitest_1.expect)(cancelCalled).toBe(true);
            (0, vitest_1.expect)(signalAbortedAfterCancel).toBe(true);
            // Wait for job to reach cancelled state
            await waitFor(async () => {
                const job = await client.getJob(handle.id, DEFAULT_TENANT);
                return job.status === client_1.JobStatus.Cancelled;
        ...
        (0, vitest_1.it)("client-side cancel notifies server immediately", async () => {
            let taskStarted = false;
            let jobStatusDuringHandler = null;
            const handler = async (ctx) => {
                taskStarted = true;
                // Wait a bit
                await new Promise((resolve) => setTimeout(resolve, 50));
        ...
        (0, vitest_1.it)("cancelling an already-completed task throws an error", async () => {
            ...
            await (0, vitest_1.expect)(handle.cancel()).rejects.toThrow("terminal state");
            // Status should still be Succeeded
            const job = await client.getJob(handle.id, DEFAULT_TENANT);
            (0, vitest_1.expect)(job.status).toBe(client_1.JobStatus.Succeeded);
        });
        (0, vitest_1.it)("multiple concurrent tasks have independent abort signals", async () => {
            ...
            // Verify the results
            (0, vitest_1.expect)(completedTasks).toContain("task-0");
            (0, vitest_1.expect)(completedTasks).toContain("task-2");
            (0, vitest_1.expect)(abortedTasks).toContain("task-1");
            // Verify job statuses
            ...
            (0, vitest_1.expect)(job0.status).toBe(client_1.JobStatus.Succeeded);
            (0, vitest_1.expect)(job1.status).toBe(client_1.JobStatus.Cancelled);
            (0, vitest_1.expect)(job2.status).toBe(client_1.JobStatus.Succeeded);
        });
        (0, vitest_1.it)("signal remains aborted after multiple abort attempts", async () => {
            ...
            const handler = async (ctx) => {
                ...
                await ctx.cancel();
                await ctx.cancel();
                (0, vitest_1.expect)(ctx.cancellationSignal.aborted).toBe(true);
                return { type: "success", result: {} };
            };
            const worker = createWorker(handler, {
                heartbeatIntervalMs: 100,
            ...
            (0, vitest_1.expect)(abortCount).toBe(1);
        });
        (0, vitest_1.it)("cancelled task does not retry", async () => {
            let attemptCount = 0;
            const handler = async (ctx) => {
                attemptCount++;
            ...
            (0, vitest_1.expect)(attemptCount).toBe(1);
            // Give some time to ensure no retry happens
            await new Promise((resolve) => setTimeout(resolve, 500));
            (0, vitest_1.expect)(attemptCount).toBe(1);
        });
        (0, vitest_1.it)("worker shutdown does not abort running tasks' cancellation signal", async () => {
            let taskStarted = false;
            let signalAborted = false;
            const handler = async (ctx) => {
                taskStarted = true;
