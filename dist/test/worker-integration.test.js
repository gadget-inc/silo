    ...
    (0, vitest_1.describe)("task cancellation", () => {
        ...
        (0, vitest_1.it)("signal remains aborted after multiple abort attempts", async () => {
            ...
            const handler = async (ctx) => {
                ...
                await ctx.cancel();
                await ctx.cancel();
                (0, vitest_1.expect)(ctx.signal.aborted).toBe(true);
                return { type: "success", result: {} };
            };
            const worker = createWorker(handler, {
                heartbeatIntervalMs: 100,
        ...
        (0, vitest_1.it)("worker shutdown aborts running tasks", async () => {
            let taskStarted = false;
            let signalAborted = false;
            let signalAbortReason = null;
            const handler = async (ctx) => {
                taskStarted = true;
                ctx.signal.addEventListener("abort", () => {
                    signalAborted = true;
            ...
            (0, vitest_1.expect)(signalAborted).toBe(true);
            (0, vitest_1.expect)(signalAbortReason).toBe("signal_aborted");
        });
    });
});
