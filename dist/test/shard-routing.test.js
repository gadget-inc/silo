...
(0, vitest_1.describe)("Shard Routing", () => {
    ...
    (0, vitest_1.describe)("Topology discovery", () => {
        ...
        (0, vitest_1.it)("cleans up stale connections after topology refresh", async () => {
            ...
            (0, vitest_1.expect)(connections.has("localhost:7450")).toBe(false);
        ...
        (0, vitest_1.it)("cleans up initial server connections when not in topology response", async () => {
        ...
        (0, vitest_1.it)("cleans up freshly-created connection when initial server fails during refresh", async () => {
            ...
            discoveredConn.client.getClusterInfo = vitest_1.vi.fn().mockReturnValue({
                response: Promise.reject(new Error("connection refused")),
            });
            ...
            client._getOrCreateConnection = (addr) => {
                const c = originalGetOrCreate(addr);
                if (addr === "localhost:7450") {
                    c.client.getClusterInfo = vitest_1.vi.fn().mockReturnValue({
        ...
        (0, vitest_1.it)("does not remove pre-existing connections when they fail during refresh", async () => {
        ...
        (0, vitest_1.it)("re-bootstraps from initial servers after all active connections are lost", async () => {
            ...
            const twoNodeTopology = vitest_1.vi.fn().mockReturnValue({
                response: Promise.resolve({
                    numShards: 2,
                    shardOwners: [
                        {
            ...
            (0, vitest_1.expect)(connections.has("seed-1:7450")).toBe(false);
            (0, vitest_1.expect)(connections.has("seed-2:7450")).toBe(false);
            (0, vitest_1.expect)(connections.has("10.0.0.1:7450")).toBe(true);
            (0, vitest_1.expect)(connections.has("10.0.0.2:7450")).toBe(true);
            ...
            const originalGetOrCreate = client._getOrCreateConnection.bind(client);
            const oneNodeTopology = vitest_1.vi.fn().mockReturnValue({
                response: Promise.resolve({
                    numShards: 2,
                    shardOwners: [
                        {
            ...
            client._getOrCreateConnection = (addr) => {
                ...
                if (addr === "seed-1:7450") {
                    ...
                    conn.client.getClusterInfo = vitest_1.vi.fn().mockReturnValue({
                        response: Promise.reject(new Error("DNS resolution failed")),
                    });
                }
                return conn;
