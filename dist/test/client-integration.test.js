...
vitest_1.describe.skipIf(!RUN_INTEGRATION)("SiloGRPCClient integration", () => {
    ...
    (0, vitest_1.describe)("topology discovery", () => {
        ...
        (0, vitest_1.it)("skips non-existent server and discovers topology from a valid one", async () => {
            ...
            const testClient = new client_1.SiloGRPCClient({
    ...
    (0, vitest_1.describe)("enqueue and getJob", () => {
        ...
        (0, vitest_1.it)("getJob returns result field for succeeded jobs", async () => {
            ...
            (0, vitest_1.expect)(scheduledJob.result).toBeUndefined();
            ...
            (0, vitest_1.expect)(succeededJob.status).toBe(client_1.JobStatus.Succeeded);
            (0, vitest_1.expect)(succeededJob.result).toEqual(resultData);
            // getJob with includeAttempts should also have result
            const jobWithAttempts = await client.getJob(handle.id, tenant, {
                includeAttempts: true,
            });
            (0, vitest_1.expect)(jobWithAttempts.status).toBe(client_1.JobStatus.Succeeded);
            (0, vitest_1.expect)(jobWithAttempts.result).toEqual(resultData);
            (0, vitest_1.expect)(jobWithAttempts.attempts).toBeDefined();
            (0, vitest_1.expect)(jobWithAttempts.attempts.length).toBeGreaterThan(0);
        });
        (0, vitest_1.it)("getJob result field is undefined for failed jobs", async () => {
            ...
            (0, vitest_1.expect)(failedJob.status).toBe(client_1.JobStatus.Failed);
            (0, vitest_1.expect)(failedJob.result).toBeUndefined();
        });
    });
    (0, vitest_1.describe)("leaseTasks and reportOutcome", () => {
        (0, vitest_1.it)("leases a task and reports success", async () => {
            ...
            const taskPayload = (0, client_1.decodeBytes)(task?.payload?.encoding.oneofKind === "msgpack"
    ...
    (0, vitest_1.describe)("query", () => {
        ...
        (0, vitest_1.it)("queries with positional bind parameters", async () => {
            ...
            const result = await client.query("SELECT id FROM jobs WHERE tenant = $1 AND id = $2", tenant, [tenant, targetId]);
            (0, vitest_1.expect)(result.rowCount).toBe(1);
            (0, vitest_1.expect)(result.rows[0]?.id).toBe(targetId);
        });
    ...
    (0, vitest_1.describe)("large payloads", () => {
        (0, vitest_1.it)("enqueues and retrieves a 5MB payload", { timeout: 60_000 }, async () => {
    ...
    (0, vitest_1.describe)("importJobs", () => {
        (0, vitest_1.it)("imports a scheduled job with no attempts", async () => {
            ...
            (0, vitest_1.expect)(results[0].id).toBe(jobId);
            (0, vitest_1.expect)(results[0].success).toBe(true);
            (0, vitest_1.expect)(results[0].status).toBe(client_1.JobStatus.Scheduled);
            // Verify the job was actually created
            const job = await client.getJob(jobId, tenant);
            (0, vitest_1.expect)(job.id).toBe(jobId);
            (0, vitest_1.expect)(job.priority).toBe(20);
            (0, vitest_1.expect)(job.payload).toEqual({ imported: true });
            (0, vitest_1.expect)(job.metadata?.source).toBe("legacy-system");
            (0, vitest_1.expect)(job.status).toBe(client_1.JobStatus.Scheduled);
        });
        (0, vitest_1.it)("imports a job with historical succeeded attempt", async () => {
            ...
            const results = await client.importJobs([
                {
                    ...
                    attempts: [
                        {
                            status: client_1.AttemptStatus.Succeeded,
            ...
            (0, vitest_1.expect)(results[0].id).toBe(jobId);
            (0, vitest_1.expect)(results[0].success).toBe(true);
            (0, vitest_1.expect)(results[0].status).toBe(client_1.JobStatus.Succeeded);
            // Verify the job has the historical attempt
            const job = await client.getJob(jobId, tenant, {
                includeAttempts: true,
            });
            (0, vitest_1.expect)(job.status).toBe(client_1.JobStatus.Succeeded);
            (0, vitest_1.expect)(job.attempts).toHaveLength(1);
            (0, vitest_1.expect)(job.attempts[0].status).toBe(client_1.AttemptStatus.Succeeded);
            (0, vitest_1.expect)(job.attempts[0].result).toEqual({ output: "done" });
        });
        (0, vitest_1.it)("imports a job with historical failed attempts and retries remaining", async () => {
            ...
            const results = await client.importJobs([
                {
                    ...
                    attempts: [
                        {
                            status: client_1.AttemptStatus.Failed,
            ...
            (0, vitest_1.expect)(job.attempts[0].status).toBe(client_1.AttemptStatus.Failed);
            (0, vitest_1.expect)(job.attempts[0].errorCode).toBe("TIMEOUT");
        });
        (0, vitest_1.it)("imports multiple jobs in a batch", async () => {
            ...
            (0, vitest_1.expect)(results[0].id).toBe(jobIds[0]);
            (0, vitest_1.expect)(results[1].id).toBe(jobIds[1]);
            (0, vitest_1.expect)(results[2].id).toBe(jobIds[2]);
            (0, vitest_1.expect)(results[0].success).toBe(true);
            (0, vitest_1.expect)(results[1].success).toBe(true);
            (0, vitest_1.expect)(results[2].success).toBe(true);
            (0, vitest_1.expect)(results[0].status).toBe(client_1.JobStatus.Scheduled);
            (0, vitest_1.expect)(results[1].status).toBe(client_1.JobStatus.Succeeded);
            (0, vitest_1.expect)(results[2].status).toBe(client_1.JobStatus.Scheduled);
        });
        (0, vitest_1.it)("imports a job with a cancelled attempt", async () => {
            ...
            const results = await client.importJobs([
                {
                    ...
                    attempts: [
                        {
                            status: client_1.AttemptStatus.Cancelled,
        ...
        (0, vitest_1.it)("imports a job with limits", async () => {
