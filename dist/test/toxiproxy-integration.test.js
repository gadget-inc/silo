...
vitest_1.describe.skipIf(!RUN_INTEGRATION)("Toxiproxy gRPC client integration", () => {
    ...
    (0, vitest_1.describe)("high concurrency enqueue", () => {
        (0, vitest_1.it)("handles many concurrent enqueues without losing requests", async () => {
            ...
            const promises = Array.from({ length: concurrency }, (_, i) => proxyClient.enqueue({
                tenant: `${tenant}-${i}`,
                taskGroup: DEFAULT_TASK_GROUP,
            ...
            (0, vitest_1.expect)(succeeded.length).toBe(concurrency);
            (0, vitest_1.expect)(failed.length).toBe(0);
        }, 30_000);
    });
    (0, vitest_1.describe)("connection reset recovery", () => {
        (0, vitest_1.it)("recovers from connection reset during enqueue", async () => {
            ...
            await (0, vitest_1.expect)(proxyClient.enqueue({
            ...
            const handle3 = await retryUntilSuccess(() => proxyClient.enqueue({
                tenant: `${tenant}-after`,
                taskGroup: DEFAULT_TASK_GROUP,
                payload: { phase: "after-reset" },
            }));
            (0, vitest_1.expect)(handle3.id).toBeTruthy();
        });
        (0, vitest_1.it)("retries and recovers from intermittent connection resets", async () => {
            ...
            for (let i = 0; i < 10; i++) {
                try {
                    const handle = await retryUntilSuccess(() => proxyClient.enqueue({
            ...
            (0, vitest_1.expect)(results.length).toBeGreaterThanOrEqual(8);
        }, 30_000);
    });
