//! FlatBuffers codec for silo's internal storage format.
//!
//! Provides encode/decode functions for all stored record types.

mod decode;
mod encode;

// Re-export public encode API
pub use crate::codec::encode::{
    encode_attempt, encode_check_rate_limit_retry_from_task, encode_concurrency_action,
    encode_floating_limit_state, encode_holder, encode_job_cancellation, encode_job_info,
    encode_job_status, encode_lease, encode_lease_from_refresh_floating_limit,
    encode_lease_from_request_ticket_grant, encode_lease_from_run_attempt, encode_task,
};

// Re-export public decode API
pub use crate::codec::decode::{
    DecodedConcurrencyAction, DecodedEnqueueTaskAction, DecodedFloatingLimitState, DecodedLease,
    DecodedTask, JobAttemptView, JobView, decode_attempt, decode_concurrency_action,
    decode_floating_limit_state, decode_holder, decode_job_cancellation, decode_job_info,
    decode_job_status, decode_lease, decode_task, decode_task_tenant, decode_task_view,
    task_is_refresh_floating_limit, validate_task,
};

// Re-export pub(crate) decode helpers
pub(crate) use crate::codec::decode::decode_floating_limit_state_bytes;
...
// --- Internal helpers used by encode.rs and decode.rs ---
...
pub(crate) fn create_string_pair_vector_from_fb<'a>(
    builder: &mut FlatBufferBuilder<'a>,
...
) -> flatbuffers::WIPOffset<flatbuffers::Vector<'a, flatbuffers::ForwardsUOffset<fb::StringPair<'a>>>>
{
    ...
    if let Some(values) = values {
        for pair in values.iter() {
            let key = builder.create_string(pair.key().unwrap_or_default());
            let value = builder.create_string(pair.value().unwrap_or_default());
            let new_pair = fb::StringPair::create(
                builder,
                &fb::StringPairArgs {
                    key: Some(key),
                    value: Some(value),
