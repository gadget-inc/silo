...
impl JobStoreShard {
    ...
    pub async fn receive_remote_ticket_request(
        ...
        use crate::codec::{
            decode_floating_limit_state, encode_floating_limit_state,
            encode_remote_concurrency_request, encode_remote_holder_record,
        };
        ...
        // For floating limits, ensure state exists on this queue owner shard
        // and potentially schedule refresh tasks
        let effective_max_concurrency = if let Some(fl) = floating_limit {
            ...
            let (state, needs_refresh_scheduling) = if let Some(state_bytes) = maybe_state {
                ...
                if let Ok(decoded) = decode_floating_limit_state(&state_bytes) {
                    ...
                    if needs_schedule {
                        let updated_state = FloatingLimitState {
                            ...
                            refresh_task_scheduled: true, // Mark as scheduled
                            retry_count: archived.retry_count,
                            next_retry_at_ms: archived
                                .next_retry_at_ms
                                .as_ref()
                                .copied(),
                            metadata: archived
                        ...
                        (Some(updated_state), true)
                    } else {
                        (None, false)
                    }
                } else {
                    // Failed to decode, treat as missing
                    (None, false)
                }
            } else {
                ...
                batch.put(state_key.as_bytes(), &state_val);
                (Some(new_state), false) // Just created, not stale yet
            };

            // Schedule refresh task if needed
            if needs_refresh_scheduling {
                // Use the state we just prepared or get current max from existing state
                let current_max = state
                    .as_ref()
                    .map(|s| s.current_max_concurrency)
                    .unwrap_or(fl.default_max_concurrency);

                let refresh_task = Task::RefreshFloatingLimit {
                    task_id: uuid::Uuid::new_v4().to_string(),
                    tenant: tenant.to_string(),
                    queue_key: queue_key.to_string(),
                    current_max_concurrency: current_max,
                    last_refreshed_at_ms: now_ms,
                    metadata: fl.metadata.clone(),
                };
                let task_val = crate::codec::encode_task(&refresh_task)?;
                batch.put(
                    crate::keys::task_key(now_ms, 0, &format!("refresh-{}", queue_key), 0)
                        .as_bytes(),
                    &task_val,
                );
                tracing::debug!(
            ...
            // Get effective max concurrency: use state's current value or the default
            state
                .map(|s| s.current_max_concurrency)
                .unwrap_or(fl.default_max_concurrency)
        } else {
