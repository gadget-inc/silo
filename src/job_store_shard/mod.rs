...
pub struct OpenShardOptions {
    ...
    /// Default terminal retention in seconds for newly created jobs.
...
impl JobStoreShard {
    ...
    pub async fn open(
        ...
        Self::open_with_resolved_store(
            ...
            OpenShardOptions {
                ...
                rate_limiter,
                metrics,
                concurrency_reconcile_interval: Duration::from_millis(5_000),
                default_terminal_retention_s: cfg.default_terminal_retention_s,
            },
            range,
        )
        .await
    ...
    pub async fn open_with_resolved_store(
        ...
        shard.spawn_terminal_cleanup_task();
    ...
    async fn delete_job_inner(&self, tenant: &str, id: &str) -> Result<(), JobStoreShardError> {
        ...
        let cleanup_task_key = match job_view.terminal_retention_s() {
            Some(retention_s) => Some(Self::terminal_cleanup_task_key(
                job_view.task_group(),
                job_view.id(),
        ...
        if let Some(key) = cleanup_task_key {
