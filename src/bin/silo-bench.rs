...
enum Command {
    /// Run a worker that discovers cluster topology and processes jobs
    Worker(WorkerArgs),
    /// Run the classic benchmark mode with manual shard configuration
    Bench(BenchArgs),
}
...
/// Worker task that polls for tasks and reports success outcomes (original implementation)
...
/// Enqueuer task that continuously enqueues tasks (original implementation)
...
async fn run_worker_mode(address: &str, args: WorkerArgs) -> anyhow::Result<()> {
    println!("Silo Benchmark Tool - Worker Mode");
    println!("==================================");
    println!("Initial server: {}", address);
    ...
    let cluster_info = discover_cluster(address).await?;
    println!(
        "Connected to node: {} ({})",
    ...
    for i in 0..args.workers {
        ...
        handles.push(tokio::spawn(worker_loop_random(
            cluster,
            worker_id,
    ...
    for i in 0..args.enqueuers {
        ...
        handles.push(tokio::spawn(enqueuer_loop_random(
            cluster,
            enqueuer_id,
            concurrency_key,
            max_concurrency,
...
async fn run_bench_mode(address: &str, args: BenchArgs) -> anyhow::Result<()> {
    ...
    let test_client = connect(address).await?;
    drop(test_client);
...
async fn main() -> anyhow::Result<()> {
    ...
    let address = args.address.as_deref().unwrap_or("http://localhost:50051");

    match args.command {
        Some(Command::Worker(worker_args)) => run_worker_mode(address, worker_args).await,
        Some(Command::Bench(bench_args)) => run_bench_mode(address, bench_args).await,
        None => {
            // Default to bench mode with default args for backwards compatibility
            run_bench_mode(
                address,
                BenchArgs {
                    workers: 8,
                    ...
                    enqueue: false,
                    enqueuers: 4,
                    concurrency_key: "bench-queue".to_string(),
                    max_concurrency: 100,
