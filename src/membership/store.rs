...
use crate::membership::mem_log_store::LogStore as MemLogStore;
use crate::membership::{
    Entry, LogId, Snapshot, SnapshotData, SnapshotMeta, StorageError, StoredMembership, TypeConfig,
};
use crate::membershippb as pb;
use crate::membershippb::Response;

pub type LogStore = MemLogStore<TypeConfig>;
...
impl RaftSnapshotBuilder<TypeConfig> for Arc<StateMachineStore> {
    ...
    async fn build_snapshot(&mut self) -> Result<Snapshot, StorageError> {
...

        {
            ...
            let last_membership_log_id = state_machine
                .last_membership_log_id
                .map(|log_id| log_id.into());
            let membership = state_machine
                .last_membership
                .clone()
                .unwrap_or_default()
                .into();
...
impl RaftStateMachine<TypeConfig> for Arc<StateMachineStore> {
        ...
        I: IntoIterator<Item = Entry>,
    {
        ...
        for entry in entries {
            ...
            if let Some(mem) = entry.membership {
