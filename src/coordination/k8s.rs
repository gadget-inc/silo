...
impl<B: K8sBackend> K8sCoordinator<B> {
    ...
    /// The optional `shard_map_seed` parameter enables deterministic shard ID generation
    /// for DST (deterministic simulation testing).
    pub async fn start_with_backend(
        ...
        Self::start_with_backend_seeded(
            backend,
            namespace,
            cluster_prefix,
            node_id,
    ...
    /// Start the K8S coordinator with a custom backend and optional seed for deterministic shard IDs.
    ///
    /// When `shard_map_seed` is provided, shard IDs will be generated deterministically
    /// based on the seed rather than using random UUIDs. This is essential for DST.
    pub async fn start_with_backend_seeded(
        backend: B,
        namespace: &str,
        cluster_prefix: &str,
        node_id: impl Into<String>,
    ...
    ///
    /// If `seed` is provided, shard IDs will be generated deterministically (for DST).
    async fn load_or_create_shard_map(
        ...
        match backend.get_configmap(namespace, &configmap_name).await? {
            ...
            None => {
                ...
                let shard_map = match seed {
                    Some(s) => ShardMap::create_initial_seeded(initial_shard_count, s),
                    None => ShardMap::create_initial(initial_shard_count),
                }
                .map_err(|e| {
    ...
    async fn run_membership_watcher(&self, mut shutdown_rx: watch::Receiver<bool>) {
        ...
        let mut watch_stream = std::pin::pin!(self.backend.watch_leases(&self.namespace, &label_selector));
        ...
        loop {
            ...
            match phase {
                ...
                ShardPhase::Releasing => {
                    ...
                    let jitter_ms =
                        (self.shard_id.as_uuid().as_u64_pair().0.wrapping_mul(17)) % 50;
                    tokio::time::sleep(Duration::from_millis(100 + jitter_ms)).await;
