...
use silo::settings::{AppConfig, DEFAULT_TERMINAL_RETENTION};
...
async fn success_cleanup_deletes_job_and_attempts() {
    let (_tmp, shard) =
        test_helpers::open_temp_shard_with_default_terminal_retention(Duration::ZERO).await;
...
#[silo::test]
async fn restart_prevents_retention_deletion() {
    // Use retention=0 so the scanner would delete immediately if the job were still terminal.
    // Use a short scan interval so the scanner runs quickly.
    let (_tmp, shard) =
        test_helpers::open_temp_shard_with_default_terminal_retention(Duration::ZERO).await;
    ...
    let job_id = shard
        .enqueue(
            "-",
            Some("job-restart-prevents-deletion".to_string()),
            50,
            0,
            None,
    ...
    // Restart the job before the scanner can delete it
    shard.restart_job("-", &job_id).await.expect("restart job");

    // Wait long enough for the scanner to have run multiple times
    tokio::time::sleep(Duration::from_millis(500)).await;

    // Job should still exist because restart moved it out of terminal state
    let restarted_status = shard
        .get_job_status("-", &job_id)
        .await
        .expect("get status")
        .expect("job should still exist after restart");
...
async fn explicit_delete_removes_terminal_job_before_retention_elapses() {
    let (_tmp, shard) =
        test_helpers::open_temp_shard_with_default_terminal_retention(Duration::from_secs(1)).await;
...
async fn importing_terminal_job_respects_retention_cleanup() {
    // Use a shard with short scan interval so the scanner picks up the imported job quickly
    let (_tmp, shard) =
        test_helpers::open_temp_shard_with_default_terminal_retention(Duration::from_secs(86400))
            .await;
