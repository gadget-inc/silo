...
async fn rate_limit_single_dequeue_returns_task() {
    ...
    let job_id = shard
        .enqueue(
            ...
            now,
            None,
            serde_json::json!({"task": "rate_limited"}),
    ...
    let tasks = shard.dequeue("-", "worker1", 1).await.expect("dequeue");
...
async fn rate_limit_exceeded_then_passes_after_window() {
    ...
    let job_id = shard
        .enqueue(
            ...
            now,
            None,
            serde_json::json!({"task": "rate_limited_retry"}),
            vec![Limit::RateLimit(rate_limit)],
            None,
        )
        .await
    ...
    let _tasks = shard.dequeue("-", "worker1", 1).await.expect("dequeue1");
...
async fn multiple_rate_limits_acquired_in_order() {
    ...
    let job_id = shard
        .enqueue(
            ...
            now,
            None,
            serde_json::json!({"task": "multi_rate_limit"}),
...
async fn concurrency_then_rate_limit_acquired_in_order() {
    ...
    let job_id = shard
        .enqueue(
            ...
            now,
            None,
            serde_json::json!({"task": "concurrency_then_rate"}),
            vec![
...
async fn rate_limit_then_concurrency_acquired_in_order() {
    ...
    let job_id = shard
        .enqueue(
            ...
            now,
            None,
            serde_json::json!({"task": "rate_then_concurrency"}),
            vec![
                Limit::RateLimit(rate_limit),
                Limit::Concurrency(concurrency),
            ],
...
async fn concurrency_blocked_with_rate_limit_pending() {
    ...
    let tasks = shard.dequeue("-", "worker1", 2).await.expect("dequeue");
    ...
    let tasks2 = shard.dequeue("-", "worker1", 1).await.expect("dequeue2");
...
async fn three_limits_concurrency_rate_concurrency() {
    ...
    let job_id = shard
        .enqueue(
            ...
            now,
            None,
            serde_json::json!({"task": "complex_limits"}),
            vec![
...
async fn rate_limit_counts_hits_correctly() {
    ...
    let tasks = shard.dequeue("-", "worker1", 5).await.expect("dequeue");
