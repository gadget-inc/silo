//! K8s worker during membership changes scenario: Tests worker invariants while
//! cluster membership is changing.
//!
//! This scenario tests that job processing invariants hold even as nodes join and
//! leave the cluster. It uses K8s coordination to manage membership while workers
//! actively process jobs.
//!
//! Fault modes:
...
//! controller adds and removes nodes from the cluster.
...
use crate::helpers::{
    ...
    report_outcome_request, run_scenario_impl, serialized_bytes,
};
...
pub fn run() {
    let seed = get_seed();
    run_scenario_impl("k8s_worker_during_membership_changes", seed, 180, |sim| {
        ...
        sim.client("controller", async move {
            ...
            for i in 0..nodes_to_add {
                ...
                // Activate the node
                if let Err(e) =
                    node_state_senders[next_node_to_activate as usize].send(NodeState::Active)
                {
            ...
            tracing::info!("controller done");
            Ok(())
        });

        // Verifier: Periodically checks DST invariants
        ...
        sim.client("verifier", async move {
            ...
            loop {
                ...
                if verifier_done_flag.load(Ordering::SeqCst) {
                    break;
                }
            }
            ...
            // With membership changes and network issues, we just need some progress
            // The key invariants are no split-brain and valid state transitions
            assert!(
