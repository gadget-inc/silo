...
async fn k8s_single_node_owns_all_shards() {
    ...
    let (coord, handle) = start_coordinator!(&namespace, &prefix, "test-node-1", "http://127.0.0.1:50051", num_shards);
...
async fn k8s_multiple_nodes_partition_shards() {
    ...
    let (c1, h1) = start_coordinator!(&namespace, &prefix, "test-node-1", "http://127.0.0.1:50051", num_shards);
    ...
    assert!(c1.wait_converged(Duration::from_secs(30)).await, "c1 should converge");
    assert!(c2.wait_converged(Duration::from_secs(30)).await, "c2 should converge");
...
async fn k8s_rebalances_on_membership_change() {
    ...
    assert!(s1.len() < num_shards as usize, "c1 should give up some shards");
...
async fn k8s_three_nodes_even_distribution() {
    ...
    // All shards covered
    let all: HashSet<u32> = s1.iter().chain(s2.iter()).chain(s3.iter()).copied().collect();
    let expected: HashSet<u32> = (0..num_shards).collect();
...
async fn k8s_removing_node_rebalances() {
    ...
    assert!(c1.wait_converged(Duration::from_secs(15)).await, "c1 should reconverge after node removal");
    assert!(c2.wait_converged(Duration::from_secs(15)).await, "c2 should reconverge after node removal");
    ...
    let all: HashSet<u32> = s1.union(&s2).copied().collect();
    let expected: HashSet<u32> = (0..num_shards).collect();
    assert_eq!(all, expected, "all shards should be covered after node removal");
...
async fn k8s_rapid_membership_churn_converges() {
    ...
    assert!(c1.wait_converged(deadline).await, "c1 should converge post-churn");
    assert!(c2b.wait_converged(deadline).await, "c2b should converge post-churn");
    assert!(c3.wait_converged(deadline).await, "c3 should converge post-churn");
...
async fn k8s_get_members_returns_correct_info() {
    ...
    assert!(member_ids.contains("member-node-1"), "should contain node-1");
    assert!(member_ids.contains("member-node-2"), "should contain node-2");
...
async fn k8s_coordinator_metadata() {
    ...
    let (coord, handle) = start_coordinator!(&namespace, &prefix, "meta-test-node", "http://192.168.1.1:9999", num_shards);
...
async fn make_k8s_guard(
    ...
    node_id: &str,
    shard_id: u32,
) -> Result<(Arc<K8sShardGuard>, Arc<Mutex<HashSet<u32>>>, tokio::sync::watch::Sender<bool>, tokio::task::JoinHandle<()>), String> {
...
async fn k8s_shard_guard_acquire_and_release() {
    ...
    let (guard, owned, _tx, handle) = match make_k8s_guard(&namespace, &prefix, "guard-node-1", shard_id).await {
        Ok(g) => g,
        Err(e) => {
...
async fn k8s_shard_guard_contention() {
    ...
    let (g1, owned1, _tx1, h1) = match make_k8s_guard(&namespace, &prefix, "contend-node-1", shard_id).await {
        Ok(g) => g,
        Err(e) => {
    ...
    let (g2, owned2, _tx2, h2) = make_k8s_guard(&namespace, &prefix, "contend-node-2", shard_id).await.unwrap();
    ...
    assert!(one_holds, "exactly one guard should acquire under contention");
...
async fn k8s_shard_guard_handoff() {
    ...
    let (g1, owned1, _tx1, h1) = match make_k8s_guard(&namespace, &prefix, "handoff-node-1", shard_id).await {
        Ok(g) => g,
        Err(e) => {
    ...
    let (g2, owned2, _tx2, h2) = make_k8s_guard(&namespace, &prefix, "handoff-node-2", shard_id).await.unwrap();
    ...
    assert!(!owned1.lock().await.contains(&shard_id), "g1 owned should be cleared");
...
async fn k8s_shard_guard_idempotent_set_desired() {
    ...
    let (guard, owned, _tx, handle) = match make_k8s_guard(&namespace, &prefix, "idemp-node", shard_id).await {
        Ok(g) => g,
        Err(e) => {
    ...
    // Still held
    assert!(guard.is_held().await, "should still be held after idempotent call");
    assert!(owned.lock().await.contains(&shard_id), "should still be in owned set");
...
async fn k8s_shard_guard_quick_flip_reacquires() {
    ...
    let (guard, owned, _tx, handle) = match make_k8s_guard(&namespace, &prefix, "flip-node", shard_id).await {
        Ok(g) => g,
        Err(e) => {
    ...
    assert!(owned.lock().await.contains(&shard_id), "owned set should include shard");
...
async fn k8s_shard_guard_acquire_aborts_on_desired_change() {
    ...
    let (g1, _o1, _tx1, h1) = match make_k8s_guard(&namespace, &prefix, "abort-node-1", shard_id).await {
        Ok(g) => g,
        Err(e) => {
    ...
    g1.set_desired(true).await;
    assert!(wait_until(Duration::from_secs(5), || async { g1.is_held().await }).await, "g1 should hold");

    // g2 starts acquiring then we cancel
    let (g2, o2, _tx2, h2) = make_k8s_guard(&namespace, &prefix, "abort-node-2", shard_id).await.unwrap();
    g2.set_desired(true).await;
    tokio::time::sleep(Duration::from_millis(100)).await;
    g2.set_desired(false).await;
    ...
    assert!(!g2.is_held().await, "g2 should not hold after aborting acquire");
    assert!(!o2.lock().await.contains(&shard_id), "g2 owned should not contain shard");
...
async fn k8s_shard_guard_shutdown_while_idle() {
    ...
    let (guard, owned, tx, handle) = match make_k8s_guard(&namespace, &prefix, "shutdown-idle", shard_id).await {
        Ok(g) => g,
        Err(e) => {
...
async fn k8s_shard_guard_shutdown_while_held() {
    ...
    let (guard, owned, tx, handle) = match make_k8s_guard(&namespace, &prefix, "shutdown-held", shard_id).await {
        Ok(g) => g,
        Err(e) => {
...
async fn k8s_no_split_brain_during_transitions() {
    ...
    assert!(!split_brain_detected, "split-brain should never occur during transitions");
...
async fn k8s_prompt_acquisition_after_node_departure() {
    ...
    assert!(
        ...
        "convergence should be reasonably prompt (was {:?})", convergence_time
    );

    let c1_final: HashSet<u32> = c1.owned_shards().await.into_iter().collect();
    let expected: HashSet<u32> = (0..num_shards).collect();
    assert_eq!(c1_final, expected, "c1 should own all shards after c2 leaves");
...
async fn k8s_multiple_add_remove_cycles() {
    ...
    let (c1, h1) = start_coordinator!(&namespace, &prefix, "cycle-node-1", "http://127.0.0.1:50051", num_shards);
    assert!(c1.wait_converged(Duration::from_secs(20)).await, "c1 initial converge");

    for i in 0..2 { // Reduced iterations
        // Add node
        ...
        assert!(c1.wait_converged(Duration::from_secs(30)).await, "c1 converge after add iter {}", i);
        assert!(c2.wait_converged(Duration::from_secs(30)).await, "c2 converge iter {}", i);
        ...
        assert!(c1.wait_converged(Duration::from_secs(30)).await, "c1 converge after remove iter {}", i);
...
async fn k8s_four_node_cluster() {
    ...
    let (c1, h1) = start_coordinator!(&namespace, &prefix, "four-node-1", "http://127.0.0.1:50051", num_shards);
    ...
    // All covered
    let all: HashSet<u32> = s1.iter().chain(s2.iter()).chain(s3.iter()).chain(s4.iter()).copied().collect();
    let expected: HashSet<u32> = (0..num_shards).collect();
