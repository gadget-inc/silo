...
Terminal jobs can also be deleted automatically by retention. By default, Silo keeps terminal jobs for 7 days before removing them, and you can override that per job with `terminal_retention_s`.
...
If the job had a pending retention-based auto-delete scheduled from its previous terminal state, restart clears that pending delete.
