...
These metrics track the background retention scanner that automatically deletes expired terminal jobs.

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `silo_retention_scans_total` | Counter | `shard` | Total number of retention scans completed |
| `silo_retention_jobs_deleted_total` | Counter | `shard` | Total number of jobs deleted by the retention scanner |

**Key insights:**
- A steady rate of `silo_retention_jobs_deleted_total` indicates the scanner is keeping up with expired jobs
- If `silo_retention_scans_total` stops incrementing, the scanner may have stalled
