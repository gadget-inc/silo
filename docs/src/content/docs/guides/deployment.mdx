...
SlateDB maintains two in-memory caches per shard to avoid re-reading data from disk or object storage:

- **Block cache**: Caches data blocks read from SST files. Reduces I/O for point lookups and scans.
- **Meta cache**: Caches SST index blocks and bloom filters. Avoids re-parsing SST indexes on each scan, which can be a significant CPU cost for scan-heavy workloads.

By default, SlateDB allocates 512 MB for the block cache and 128 MB for the meta cache per shard. For workloads with high scan rates (many background scans, large numbers of SST files), increasing the meta cache can reduce CPU spent on SST index deserialization:

```toml
[database.memory_cache]
block_cache_bytes = 536870912   # 512 MB (default)
meta_cache_bytes = 268435456    # 256 MB (doubled from 128 MB default)
```

To determine if the meta cache is undersized, look for high CPU usage in SST index parsing (visible in CPU profiles as `SsTableIndexOwned::new` or `TableStore::read_index`). Increasing the meta cache keeps more parsed indexes in memory and avoids repeated flatbuffer deserialization and CRC verification on cache misses.

## Garbage collection

**SlateDB does not garbage collect old files by default.** Without explicit configuration, old WAL files, compacted SSTs, manifest versions, and compaction job state will accumulate in storage indefinitely. For production deployments, you should always enable garbage collection.

Add the following to your Silo config to enable GC for all file categories:
...
The `interval` controls how often the GC scans for old files, and `min_age` is the minimum age before a file is eligible for deletion. The WAL `min_age` can be shorter (30 minutes) because WAL files are superseded quickly by L0 SSTs, while manifests and compacted SSTs use a longer `min_age` (24 hours) as a safety margin to avoid deleting files that might still be referenced by active readers or checkpoints.

See the [Server Configuration reference](/silo/reference/server-configuration/#garbage-collector-options) for full details on all GC options.
