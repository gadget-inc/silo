...
You can enqueue a job by enqueuing a payload:
...
const client = new SiloGRPCClient({
  ...
  servers: "localhost:50051",
  // or multiple servers: ["server1:50051", "server2:50051"]
  // or as object: { host: "localhost", port: 50051 }
...
const urgentHandle = await client.enqueue({
  ...
  limits: [{ type: "concurrency", key: "notifications", maxConcurrency: 10 }]
});
...
<Aside type="caution" title="Priority requires a concurrency limit">
Priority only affects ordering within concurrency queues. Without concurrency limits, tasks are processed in time order (earliest `startAtMs` first). If you specify a non-default priority without a concurrency limit, the enqueue will fail.

To use priority-based ordering, add a concurrency limit to your job. Jobs waiting for the same concurrency queue will be granted slots in priority order.
</Aside>
...
<Aside type="tip" title="Priority ordering">
When multiple jobs are waiting for the same concurrency queue, they are granted slots in **priority order** (lower priority number first). This is the only way to implement priority-based scheduling in Silo. See the [priority option](#priority-optional) above.
</Aside>
...
3. Configure [server settings](/guides/server-configuration) for production
