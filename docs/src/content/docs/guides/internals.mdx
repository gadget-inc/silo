...
- A `Scheduled` job on the holder's own attempt is a chain parked between limits, and an owner-unknown record has nothing to check: both are **live** until they pass `orphan_holder_stale_ms`, after which a lease-less holder is an orphan regardless of owner.

A holder is purged only after two consecutive full passes classify it as an orphan, so a snapshot racing an in-flight grant or lease write never releases a live slot. Purging deletes the durable row, releases the in-memory reservation, and wakes the grant scanner for the queue so the head requester is admitted. Every purge is logged at `warn` with the tenant, queue, task id, owner, and reason, and counted in `silo_concurrency_orphan_holders_purged_total{shard, reason}`.

The stale rule trades safety for recovery: a chain parked longer than the threshold, or a worker whose lease lapsed before the reaper ran, can lose its holder and the queue over-admits by one slot. That is the same tradeoff the console's Drop holders action makes, applied to one holder at a time. The sweep never touches leases, tasks, requests, or job status.
