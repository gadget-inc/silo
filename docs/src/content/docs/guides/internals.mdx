...
3. **SplitCloning** — The underlying SlateDB database is cloned for both child shards.
4. **SplitUpdatingMap** — The shard map is atomically updated to replace the parent with the two children.
5. **SplitComplete** — Children are active and ready to serve traffic.
...
For crash recovery, splits in early phases (before the shard map is updated) are abandoned and the parent remains intact. Splits in late phases (after children exist in the shard map) can be resumed to completion.
