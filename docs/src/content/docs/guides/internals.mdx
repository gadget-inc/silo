...
- The inverted timestamp in status/time index uses `u64::MAX - timestamp` for newest-first ordering

The lease record value stores `{ worker_id, task, expiry_ms }`. Workers must heartbeat or complete before expiry to retain ownership. Expired lease detection scans the lease prefix and inspects `expiry_ms`.

See `src/keys.rs` for the full implementation and parsing utilities.

## Clustering approach

Silo works by separating the compute and storage tiers. Storage is provided by object storage, and the compute nodes are ephemeral -- they can be scaled up and down elastically. Data durability comes from object storage, not from the compute nodes themselves.
...
When using a local WAL (write-ahead log) for performance, shard leases are **permanent** -- they persist through node crashes to prevent WAL data loss. See [Permanent shard leases](#permanent-shard-leases) below for details.
...
- **Ownership (permanent)**: The active owner of shard `<S>` is the node that holds the etcd key `coord/shards/<S>/owner`. Value: `{ owner_id, owner_addr, acquired_at }`. Unlike membership, shard ownership keys are **not** tied to any TTL-based lease -- they persist until explicitly released (graceful shutdown) or force-released (operator action).
...
- Membership lease TTL: 10s; keepalive: every 2s; optional quiesce deadline: 1–2s per shard.
- Watch backoff with jitter to avoid thundering herds on membership changes.

Key layout (control plane):

- `coord/members/<node_id>` → ephemeral (tied to membership lease TTL); node metadata
- `coord/shards/<shard_id>/owner` → permanent (no TTL); ownership record
