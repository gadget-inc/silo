...
- `tasks/<start-time>/<job-id>/<attempt-number>` - stores the work items that need accomplishing in task order. Each stored task has a system-generated UUID. Priority ordering is handled by concurrency queues, not task key ordering.
