...
<Aside type="danger">
Force-releasing a shard lease may cause data loss if the original node still has unflushed WAL data on local disk. Only use this when you are certain the node is permanently lost and will not come back.
</Aside>
