...
# Optional: orphan concurrency holder sweep (see the per-option sections below)
...
On the same reconciliation loop, each shard walks its durable concurrency holder rows and releases holders whose task is gone, so a leaked holder cannot wedge a queue indefinitely. The [Internals guide](/silo/guides/internals/#orphan-holder-sweep) describes how holders are classified. This option caps how many holder rows one tick walks; the sweep resumes from a cursor on the next tick, so a full pass over a shard's holders spans several ticks. Defaults to `5000`; values below 1 are treated as 1.

#### `orphan_holder_grace_ms`

A concurrency holder younger than this many milliseconds is never classified by the orphan holder sweep, so a freshly granted holder whose lease or task write is still landing is left alone. Defaults to `60000`.

#### `orphan_holder_stale_ms`

A concurrency holder with no unexpired lease that is older than this many milliseconds is purged regardless of its job's status. This recovers holders whose record carries no owner, and chains parked or tasks queued for longer than any legitimate wait, at the cost of a single over-admit on that queue. Defaults to `86400000` (24 hours). `0` disables this age-only rule, and holders whose record carries no owner are then never purged.
