...
| `default_terminal_retention` | duration | `"7d"` | Default retention for jobs after they reach a terminal state. Per-job `terminal_retention_ms` overrides this. |
...
- Can be overridden per job with `terminal_retention_ms`
- `"0s"` means delete immediately after the job reaches a terminal state
