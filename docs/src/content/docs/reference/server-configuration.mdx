...
Silo enables garbage collection by default (SlateDB's own default is to disable it). The GC cleans up old manifest files, WAL segments, compacted SSTs, and compaction state files.
