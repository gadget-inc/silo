...
# Optional: default retention for terminal jobs (7 days)
# default_terminal_retention_s = 604800
...
| `default_terminal_retention_s` | number | `604800` | Default retention in seconds for jobs after they reach a terminal state. Per-job `terminal_retention_s` overrides this. |
...
Controls how long terminal jobs are retained before Silo automatically deletes them.

- Applies to `Succeeded`, `Failed`, and `Cancelled` jobs
- The effective value is stored on the job when it is created
- Can be overridden per job with `terminal_retention_s`
- `0` means delete immediately after the job reaches a terminal state
