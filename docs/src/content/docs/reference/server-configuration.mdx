...
# Optional: orphan concurrency holder sweep (see "Orphan Holder Sweep" below)
# orphan_holder_sweep_slice = 5000
# orphan_holder_grace_ms = 60000
# orphan_holder_stale_ms = 86400000
...
| `orphan_holder_sweep_slice` | number | `5000` | Max durable concurrency holder rows the orphan holder sweep walks per reconcile tick; values below 1 are treated as 1 |
| `orphan_holder_grace_ms` | number | `60000` | Minimum age of a concurrency holder before the orphan holder sweep classifies it |
| `orphan_holder_stale_ms` | number | `86400000` | Age past which a concurrency holder with no unexpired lease is purged regardless of its job's status; `0` disables this rule |
...
Each shard periodically walks its durable concurrency holder rows and releases holders whose task is gone, so a leaked holder cannot wedge a queue indefinitely. The internals guide's "Orphan holder sweep" section describes how holders are classified. Three options tune the sweep:

- `orphan_holder_sweep_slice` (default `5000`): holder rows walked per concurrency-reconcile tick. The sweep resumes from a cursor on the next tick, so a full pass over a shard's holders spans several ticks, and a holder is purged only after two consecutive full passes classify it as an orphan. Values below 1 are treated as 1.
- `orphan_holder_grace_ms` (default `60000`): a holder younger than this is never classified, so a freshly granted holder whose lease or task write is still landing is left alone.
- `orphan_holder_stale_ms` (default `86400000`, 24 hours): a holder with no unexpired lease older than this is purged regardless of its job's status. This recovers holders whose record carries no owner and chains parked for longer than any legitimate wait, at the cost of a single over-admit on that queue. `0` disables this age-only rule, and holders whose record carries no owner are then never purged.
