...
-------------------------------------------------------------------------------
-- State Relations (indexed by Time)
-------------------------------------------------------------------------------
...
pred dequeue[tid: TaskId, w: Worker, a: Attempt, t: Time, tnext: Time] {
    ...
    let j = bufferedAt[tid, t] | {
        ...
        (not isTerminal[statusAt[j, t]]) implies {
            -- Active job: create lease, mark running, create attempt
            one l: Lease | l.ltask = tid and l.lworker = w and l.ljob = j and l.lattempt = a and l.ltime = tnext
